...
## 二十八、Cloudflare 部署與發布規範（極重要）

為了確保專案能持續穩定地發布至 Cloudflare，後續的所有部署與環境設定**必須嚴格遵守**以下規範。違規將導致構建階段的 `ASSETS` 綁定衝突與部署失敗。

### 28.1 核心配置規範 (Configuration Rules)

> [!WARNING]
> **嚴禁在 `wrangler.jsonc` 中加入 `pages_build_output_dir` 屬性。**

**原因**：
當使用 `@astrojs/cloudflare` adapter 時，Astro 會在底層自動化處理編譯路徑與靜態資源（ASSETS）綁定。如果在 `wrangler.jsonc` 中手動加上 `"pages_build_output_dir": "./dist/client"`，會強制觸發 Wrangler 的 Pages 專案嚴格校驗模式，這將與 Astro 自動生成的內部 `ASSETS` 綁定發生衝突，導致拋出 `The name 'ASSETS' is reserved in Pages projects` 致命錯誤並中斷編譯。

**正確的 `wrangler.jsonc` 範例**：
```jsonc
{
...
### 28.2 發布流程 (Deployment Workflow)

專案已將建置與部署腳本整合，請唯一使用以下指令進行線上發布：
...
**該指令底層執行動作**：
1. `npm run build:fix`：觸發 `astro build` 產出伺服器與客戶端靜態資源。
2. 自動執行 Node 腳本，清理 `.prerender` 目錄下內部產生的 `wrangler.json` 中殘留的無效 `assets` 屬性。
3. `wrangler deploy`：讀取根目錄的 `wrangler.jsonc`，將產物作為 Workers 站點發布至 Cloudflare。

### 28.3 主題與視覺風格

> [!IMPORTANT]
> **本專案嚴格採用「Golden Rainforest（沉浸式黑金叢林）」為唯一主題風格。**

- **無 Light Mode**：本項目已在先前的架構審計後，確認完全移除所有明亮模式（Light Mode）的代碼與切換按鈕。
- **純粹深色模式**：請勿在 `Layout.astro` 再次引入基於系統偏好的 `prefers-color-scheme: light` 切換邏輯。保持預設且唯一的全域深色渲染。
